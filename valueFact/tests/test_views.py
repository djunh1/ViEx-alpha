...
class newStockSearchTest(SimpleTestCase):
	...
	def test_validation_errors_are_on_stocks_page(self):
		response = self.client.post('/companies/', data={'text': ''})
	...
	def test_is_correct_ticker_being_used(self):
		...
		response=self.client.post('/companies/', data={'text': 'SDRL'})
	...
	def test_stock_data_freeCashFlow_fetching(self):
		...
		response=self.client.post('/companies/', data={'text': 'NOV'})
	...
	def test_stock_data_ncav_fetching(self):
		...
		response=self.client.post('/companies/', data={'text': 'WAT'})
	...
	def test_data_has_none(self):
		response = self.client.post('/companies/', data={'text': 'ASAX'})
	'''
